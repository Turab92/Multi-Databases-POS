      ...
      else 
      {
        ...
        Wh_po_det.findAll({
          ...
          .then((data) => {
            ...
            {
              res.status(409).send({
                message: "Material Already Exist with this Purchase Order Master id"+req.body.wh_po_mid,
              });
            }
          })
          .catch((err) => {
...
//           attributes: [[sequelize.fn('SUM', sequelize.col('recv_unit_qty')), 'recv_qty',],
//           [sequelize.literal(`(Select price from wh_grn_details where wh_grn_details."material_id"=${det.dataValues.material_id} and "wh_grn_details"."dept_id"=${req.params.dept_id} and wh_grn_details.status=1 ORDER BY wh_grn_details.wh_grn_did DESC LIMIT 1 )` ),`pr`],
//           [sequelize.literal(`(SELECT SUM("wh_so_details"."so_unit_qty") from "wh_so_details" where "wh_so_details"."material_id"=${det.dataValues.material_id} and "wh_so_details"."par_dept_id"=${req.params.dept_id})`),`so_qty`],
//           ],
//         });
//           if(!data.length)
//           {
